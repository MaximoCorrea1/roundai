...
| 2026-06-06 | Goal-screen yield shown as a 12-month PROJECTION ("rendiría… ✦ simulado", conditional tense) instead of accrued-to-date | A just-activated account has honestly \$0 accrued — precise but reads dead in the demo; the projection is equally honest (conditional + simulado) and lands the wow. Ring/remaining math still counts only accrued amounts. |
| 2026-06-06 | Demo path LOCKED: quiz m/m/m → $500.000 → 12 meses → tweaker to 7% → confirm in tweaker. Script numbers (+$305, $321.845, $82.906, 6 meses) hold ONLY on this path | Margin is tweaker-path-dependent (snap grid 0.5% vs exact comodo fraction); mixing paths mixes number sets in front of judges. |
